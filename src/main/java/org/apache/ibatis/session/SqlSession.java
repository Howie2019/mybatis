 ...
 * <p>
 * 这是MyBatis主要的一个类，用来执行SQL，获取映射器(mapper)，管理事务<br/>
 * 通常情况下，我们在应用程序中使用的Mybatis的API就是这个接口定义的方法。
 */
//SqlSession都持有一个Executor
public interface SqlSession extends Closeable {
    ...
    /**
     * 得到的Mapper持有当前的SqlSession, 这个Mapper就是当前SqlSession的代理.
     * 我们不再直接通过SqlSession操作数据库, Mapper挡在SqlSession前面, 我们通过Mapper操作数据库
     * <p>
     * 最终通过Proxy.newProxyInstance(,,)生成代理对象,
     * <p>
     * 当然反射性能低, 这里先从MapperRegistry获取缓存的代理对象
     * <p>
     * 这个巧妙的使用了泛型，使得类型安全, 到了MyBatis 3，还可以用注解,这样xml都不用写了
     *
     * @param <T>  the mapper type
     * @param type XXXMapper.class(XXXMapper里面定义了各种增删改查方法)
