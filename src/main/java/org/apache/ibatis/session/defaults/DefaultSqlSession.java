...
public class DefaultSqlSession implements SqlSession {
    ...
    /**
     * SqlSession只是一个MyBatis对外的接口，SqlSession将它的工作交给了Executor执行器这个角色来完成
     */
    ...
    //核心selectOne
    @Override
    public <T> T selectOne(String statementIdentifier, Object parameter) {
        ...
        List<T> list = this.<T>selectList(statementIdentifier, parameter);
        if (list.size() == 1) {
            return list.get(0);
        } else if (list.size() > 1) {
    ...
    public <K, V> Map<K, V> selectMap(String statement, Object parameter, String mapKey, RowBounds rowBounds) {
        ...
        final DefaultMapResultHandler<K, V> mapResultHandler = new DefaultMapResultHandler<K, V>(mapKey, configuration.getObjectFactory(), configuration.getWrapperFactory());
    ...
    /**
     * 这里得到的Mapper其实就是SqlSession对象的代理
     * <p>
     * 这里方法把当前对象自己作为参数去调用configuration.getMapper.
     */
    @Override
    public <T> T getMapper(Class<T> type) {
        //最后会去调用MapperRegistry.getMapper
        //注意: 这里方法把当前对象自己作为参数去调用configuration.getMapper
