...
public class MapperBuilderAssistant extends BaseBuilder {
    ...
    public String applyCurrentNamespace(String base, boolean isReference) {
        ...
        if (isReference) {
            ...
            if (base.contains(".")) {//如果包含. 说明已经有namespace前缀了, 这时候直接返回base
                return base;
            }
        } else {
            ...
            if (base.contains(".")) {//如果base没有被currentNamespace.修饰, 但是
     ...
     * 根据namespace得到Cache.
     *
     * @param namespace 是一个key, 用于从map中获取Cache
     */
    ...
    public Cache useNewCache(Class<? extends Cache> typeClass, Class<? extends Cache> evictionClass, Long flushInterval, Integer size, boolean readWrite, boolean blocking, Properties props) {
        ...
        Cache cache = new CacheBuilder(currentNamespace).implementation(typeClass).addDecorator(evictionClass).clearInterval(flushInterval).size(size).readWrite(readWrite).blocking(blocking).properties(props).build();
        //加入缓存
    ...
    public MappedStatement addMappedStatement(String id, SqlSource sqlSource, StatementType statementType, SqlCommandType sqlCommandType, Integer fetchSize, Integer timeout, String parameterMap, Class<?> parameterType, String resultMap, Class<?> resultType, ResultSetType resultSetType, boolean flushCache, boolean useCache, boolean resultOrdered, KeyGenerator keyGenerator, String keyProperty, String keyColumn, String databaseId, LanguageDriver lang, String resultSets) {
        ...
        //又是建造者模式
        MappedStatement.Builder mappedStatementBuilder = new MappedStatement.Builder(configuration, id, sqlSource, sqlCommandType);
        mappedStatementBuilder.resource(resource);
        mappedStatementBuilder.fetchSize(fetchSize);
        mappedStatementBuilder.statementType(statementType);
        mappedStatementBuilder.keyGenerator(keyGenerator);
        mappedStatementBuilder.keyProperty(keyProperty);
        mappedStatementBuilder.keyColumn(keyColumn);
        mappedStatementBuilder.databaseId(databaseId);
        mappedStatementBuilder.lang(lang);
        mappedStatementBuilder.resultOrdered(resultOrdered);
        mappedStatementBuilder.resulSets(resultSets);
        setStatementTimeout(timeout, mappedStatementBuilder);//mappedStatementBuilder.timeout(timeout);
        //1.参数映射
        setStatementParameterMap(parameterMap, parameterType, mappedStatementBuilder);
        //2.结果映射
        setStatementResultMap(resultMap, resultType, resultSetType, mappedStatementBuilder);
        setStatementCache(isSelect, flushCache, useCache, currentCache, mappedStatementBuilder);
    ...
    private void setStatementParameterMap(String parameterMap, Class<?> parameterTypeClass, MappedStatement.Builder mappedStatementBuilder) {
        parameterMap = applyCurrentNamespace(parameterMap, true);

        if (parameterMap != null) {
            try {
                mappedStatementBuilder.parameterMap(configuration.getParameterMap(parameterMap));
            } catch (IllegalArgumentException e) {
                throw new IncompleteElementException("Could not find parameter map " + parameterMap, e);
            }
        } else if (parameterTypeClass != null) {
            List<ParameterMapping> parameterMappings = new ArrayList<ParameterMapping>();
            ParameterMap.Builder inlineParameterMapBuilder = new ParameterMap.Builder(configuration, mappedStatementBuilder.id() + "-Inline", parameterTypeClass, parameterMappings);
            mappedStatementBuilder.parameterMap(inlineParameterMapBuilder.build());
        }
    }
    ...
    private void setStatementResultMap(String resultMap, Class<?> resultType, ResultSetType resultSetType, MappedStatement.Builder statementBuilder) {
        ...
        if (resultMap != null) {
            ...
            ResultMap.Builder inlineResultMapBuilder = new ResultMap.Builder(configuration, statementBuilder.id() + "-Inline", resultType, new ArrayList<ResultMapping>(), null);
    ...
    /**
     * 在statementBuilder.timeout(timeout);之前加入了null判断
     */
